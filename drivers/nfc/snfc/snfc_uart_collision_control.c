...
static long snfc_uart_control_ioctl(struct file *flip, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case IOCTL_SNFC_START_AUTOPOLL :
			...
			autopoll_status = 1;
#if 0
			while(GPIO_LOW_VALUE == snfc_gpio_read(GPIO_SNFC_RFS))
			{
				autopoll_wait_cnt++;
				if(autopoll_wait_cnt/50)
				{
					break_cnt++;
					SNFC_DEBUG_MSG("[snfc_uart_control] IOCTL_SNFC_START_AUTOPOLL - rfs high\n");
					autopoll_wait_cnt = 0;
					if(break_cnt >=10)
						break;
				}
...
static int snfc_uart_control_read(struct file *pf, char *pbuf, size_t size, loff_t *pos)
{
	...
	#ifdef FEATURE_DEBUG_LOW 
	SNFC_DEBUG_MSG("[snfc_uart_control] snfc_uart_control_read :koto_abnormal=%d - end \n",koto_abnormal);
	#endif	
...
static int snfc_uart_control_write(struct file *pf, const char *pbuf, size_t size, loff_t *pos)
{
	...
	#ifdef FEATURE_DEBUG_LOW 
	SNFC_DEBUG_MSG("[snfc_uart_control] snfc_uart_control_write - end:koto_abnormal=%d \n",koto_abnormal);
	#endif	
